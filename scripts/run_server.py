...
async def inference_start(sid, data):
    ...
    try:
        ...
        if not model_id or not layer_name or not transfer_id or not total_chunks:
            ...
            if not model_id: missing.append("model_id")
            if not layer_name: missing.append("layer_name")
            if not transfer_id: missing.append("transfer_id")
            if not total_chunks: missing.append("total_chunks")
            error_msg = f"Missing required parameters: {', '.join(missing)}"
            logger.error(error_msg)
            return {"error": error_msg}, 400
        
        logger.info(f"Beginning chunked inference, model {model_id}, layer {layer_name}")
        
        if sid not in chunk_managers:
            chunk_managers[sid] = ChunkManager()
        
        chunk_managers[sid].register_transfer(transfer_id, total_chunks, {
            "model_id": model_id,
...
async def chunk(sid, data):
    ...
    try:
        ...
        if not transfer_id or chunk_index < 0 or not total_chunks or not chunk_data:
            missing = []
            if not transfer_id: missing.append("transfer_id")
            if chunk_index < 0: missing.append("chunk_index")
            if not total_chunks: missing.append("total_chunks")
            if not chunk_data: missing.append("data")
            error_msg = f"Missing required parameters: {', '.join(missing)}"
            logger.error(error_msg)
            return {"error": error_msg}, 400
