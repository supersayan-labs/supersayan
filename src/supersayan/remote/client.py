...
class SupersayanClient(SupersayanModel):
        ...
        try:
            ...
            if status_code != 200:
                if isinstance(data, dict) and "error" in data:
                    ...
                    raise ValueError(
                        f"Failed to upload model with status code: {status_code}"
                    )
    ...
    def _get_remote_layer_names(self):
        ...
        response = self.sio.call(
            ...
            timeout=MODEL_STRUCTURE_TIMEOUT,
        )
        ...
        if status_code != 200:
            if isinstance(data, dict) and "error" in data:
                ...
                raise ValueError(
                    f"Failed to retrieve model structure with status code: {status_code}"
                )
    ...
    def _send_chunked_data(
        ...
        data: Dict[str, Any],
        large_data_key: str,
        timeout: int = STANDARD_CHUNK_TIMEOUT,
        ...
        start_data = {**metadata, "chunked": True, large_data_key: None}
        ...
        start_response = self.sio.call(
            f"{event_name}_start", start_data, timeout=timeout
        )
        ...
        for chunk in chunks:
            ...
            if chunk_status != 200:
                logger.error(
                    f"Failed to send chunk {chunk['chunk_index'] + 1}/{chunk['total_chunks']}"
                )
                return chunk_response

        logger.info(
            f"All chunks sent for transfer {transfer_id}, waiting for completion"
        )
        return self.sio.call(
            f"{event_name}_complete", {"transfer_id": transfer_id}, timeout=timeout * 2
        )

    def _process_layer(
        self, layer_name: str, encrypted_input: np.ndarray
    ) -> np.ndarray:
        """
        ...
        try:
            ...
            response = self._send_chunked_data(
                "inference",
                {
                    "model_id": self.model_id,
            ...
            if status_code != 200:
                if isinstance(data, dict) and "error" in data:
                    ...
                    raise ValueError(
                        f"Layer processing failed with status code: {status_code}"
                    )
            ...
            if encrypted_output_base64 is None and data.get("chunked", False):
                ...
                def get_chunk(chunk_index):
                    ...
                    if chunk_status != 200:
                        raise ValueError(
                            f"Failed to retrieve chunk {chunk_index + 1}/{total_chunks}"
                        )

                    return chunk_data

                encrypted_output_base64 = self.chunk_manager.receive_chunked(
                    transfer_id, total_chunks, get_chunk
                )

                try:
                    self.sio.call(
                        "cleanup_response_chunks",
    ...
    def _hybrid_forward_module(
        self, prefix: str, module: torch.nn.Module, x: torch.Tensor
    ) -> torch.Tensor:
        """
