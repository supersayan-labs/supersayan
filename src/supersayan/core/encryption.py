...
@overload
def encrypt(
    mu: torch.Tensor, key: List[float], sigma: Optional[float] = None
) -> np.ndarray: ...


def encrypt(
    mu: Union[float, torch.Tensor], key: List[float], sigma: Optional[float] = None
) -> Union[LWE, np.ndarray]:
    """
    ...
    if isinstance(mu, float) or isinstance(mu, int):
        try:
            if sigma is not None:
                result = SupersayanTFHE.Encryption.encrypt_torus_to_lwe(
                    float(mu), key, sigma
                )
            else:
    ...
    # Handle tensor
        ...
        try:
            if sigma is not None:
                encrypted_flat = SupersayanTFHE.Encryption.encrypt_torus_to_lwe_vec(
                    mu_np_flat, key, sigma
                )
            else:
                encrypted_flat = SupersayanTFHE.Encryption.encrypt_torus_to_lwe_vec(
                    mu_np_flat, key
        ...
        python_lwe_objects = np.array(
            [LWE.from_julia(lwe) for lwe in encrypted_flat], dtype=object
        )
    ...
    # Handle unsupported input type
        ...
        raise TypeError(
            f"Cannot encrypt object of type {type(mu)}. Expected float or torch.Tensor."
        )
...
def decrypt(
    ciphertext: Union[LWE, np.ndarray], key: List[float], p: int = 5
    ...
    # Handle numpy array (any dimension)
        ...
        try:
            ...
            decrypted_values = SupersayanTFHE.Encryption.decrypt_lwe_to_torus_vec(
                flat_ciphertexts, key, p
            )
        except Exception as e:
    ...
    # Handle unsupported input type
        ...
        raise TypeError(
            f"Cannot decrypt object of type {type(ciphertext)}. Expected LWE or numpy.ndarray."
        )
